...
export const actions: Actions = {
  default: async ({ request, cookies }) => {
    ...
    if (
      !title || !artistName || !artistApiId || !priceRaw || !productCode ||
      !releaseDate
    ) {
      console.log("Faltando campos obrigatórios");
      return fail(400, {
        error:
          "Preencha todos os campos obrigatórios (Nome, Artista, ID do Artista, Preço, Código e Data).",
      });
    }
    ...
    try {
      ...
      if (err?.status === 401) {
        return fail(401, {
          error: "Autenticação inválida. Faça login novamente.",
        });
      }
      return fail(500, {
        error: err?.message || "Erro interno ao registrar o produto.",
      });
    }
